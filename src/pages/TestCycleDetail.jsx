    ...
    <div style={{ display: "flex", minHeight: "100vh", backgroundColor: "#f8fafc" }}>
      ...
      <main style={{ marginLeft: "220px", flex: 1, padding: "32px", maxWidth: "1420px" }}>
        ...
        <div style={{ marginTop: "24px", padding: "12px 0", borderTop: "1px solid #e2e8f0", display: "flex", gap: "24px", fontSize: "11px", color: "#94a3b8" }}>
          <span>Created: {cycle?.created_at ? new Date(cycle.created_at).toLocaleString() : "—"}</span>
          <span>Updated: {cycle?.updated_at ? new Date(cycle.updated_at).toLocaleString() : "—"}</span>
          <span style={{ fontFamily: "monospace", fontSize: "10px", cursor: "pointer" }} onClick={() => { navigator.clipboard.writeText(id); toast && toast("ID copied to clipboard") }} title="Click to copy full ID">ID: {cycle?.id?.substring(0, 8)}</span>
        </div>
      </main>
    </div>
  )
